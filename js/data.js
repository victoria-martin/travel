...
const ACCOMMODATION_TYPES = {
  hotel:        { label:'Hôtel',         short:'Hôtel',  emoji:'🏨', color:'#A6462E', tagClass:'tag-hotel', aliases:['hotel','hôtel'] },
  homeExchange: { label:'Home exchange', short:'HE',     emoji:'🔁', color:'#7C8B5E', tagClass:'tag-home',  aliases:['home','exchange','échange'] },
  house:        { label:'Maison',        short:'Maison', emoji:'🏡', color:'#3E6259', tagClass:'tag-house', aliases:['maison','house'] },
};

function accType(type){ return ACCOMMODATION_TYPES[type] || ACCOMMODATION_TYPES[DEFAULT_ACCOMMODATION_TYPE]; }

function accTypeFromText(text){
  const lower = (text||'').toLowerCase();
  const found = Object.keys(ACCOMMODATION_TYPES).find(key => accType(key).aliases.some(a => lower.includes(a)));
...
let mapFilters = { types:new Set(Object.keys(ACCOMMODATION_TYPES)), regions:new Set(), scenarioId:null, favOnly:false };
let leafletMap = null;
let saveTimer = null;
