function emptyListItem(kind) {
  const empty = { id: null };
  LIST_CONFIG[kind].fields.forEach((f) => (empty[f.key] = ''));
  return empty;
}

function listForm(kind, p) {
  ...
  return /* HTML */ `
    ...
    <div class="modal-actions">
      ...
      <button class="btn" onclick="saveListItem('${kind}','${p.id || ''}')">Enregistrer</button>
    </div>
  `;
}
