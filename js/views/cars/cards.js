...
function carCard(car) {
  return /* HTML */ `<div class="card">
    <div class="card-top">
      <p class="card-name">${escapeHtml(providerName(car.providerId)) || 'Sans nom'}</p>
      ${defaultCarCell(car)}
    </div>
    <div class="card-meta">
      ${car.model ? `<span>Modèle : ${escapeHtml(car.model)}</span>` : ''}
      ${car.pricePerDay ? `<span>${escapeHtml(typedPriceLabel(car.pricePerDay, '/ jour'))}</span>` : ''}
      ${car.priceTotal ? `<span>Total : ${escapeHtml(typedPriceLabel(car.priceTotal))}</span>` : ''}
