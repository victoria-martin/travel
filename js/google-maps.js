/*
  Coller un lien Google Maps dans la modale activité pré-remplit la fiche.
...
function importGoogleMapsPaste() {
  setTimeout(importGoogleMapsLink, 0);
}

// Pasting imports right away, so leaving the field must not read the same place twice.
async function importGoogleMapsLink() {
  const field = document.getElementById('a-maps-link');
  ...
  try {
    applyGoogleMapsPlace(await fetchGoogleMapsPlace(url));
  } catch (e) {
...
function applyGoogleMapsPlace(place) {
  const filled = GOOGLE_MAPS_FIELDS.filter(({ key, id }) => {
